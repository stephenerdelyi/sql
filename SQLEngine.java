...
public class SQLEngine extends SQL {
    String databaseLocation = System.getProperty("user.home") + "/cs457/"; //the location of the database, currently set to ~/cs457/paN/
    ...
    Table saveTables[] = new Table[15]; //the array of tables that can be internally saved before a commit
    int numSavedTables = 0; //the index of tables that have been saved
    boolean usingTransactions = false; //whether or not we are using transactions in our current environment
    ...
    public void update(Token token) {
        if(currentDB == "") {
            ...
            if(usingTransactions) {
                //if a lock is already acquired
                //BUG: Will need to check if the current table is stored in the saveTables array. If it is, then local table variable should be reset to this value
                //BUG: Will need to assign random integer generated by process to each table_lock file to verify that the process that created the lock is editing it
                if(lockExists(token.tblName)) {
                    console.warn("Error: Table " + token.tblName + " is locked!");
                    return;
                } else {
                    table.lock.acquire();
                }
            ...
            if(updateValue > 0) {
                if(usingTransactions) {
                    saveTables[numSavedTables] = table;
                    numSavedTables++;
                } else {
                    table.export();
                }
    ...
    //beginTransaction - enables transactions in the system
    ...
    //lockExists - returns whether or not the specified lock exists in the system
    public boolean lockExists(String tblName) {
        if(readFile.fileExists(databaseLocation + currentDB + "/" + tblName + "_lock")) {
            return true;
        }
        return false;
    }
